...
sizes = [10, 30, 20]
weights = [[size/float(sum(sizes))]*n_rolls for size in sizes]

plot = plt.hist([rolls3,rolls2,rolls1], n_bins, range=[x_min, x_max], histtype='stepfilled', weights=weights, stacked=True,
                label=["2 150-sided dice, size 1","15 30-sided dice, size 3","20 8-sided dice, size 2"])
axes = plt.gca()
ymax = axes.get_ylim()[1]
axes.set_ylim([0,(n_rolls+1)/float(n_bins/6.)])
...
for n in range(n_events):
    choice = np.random.randint(1,high=sum(sizes)+1)
    if choice <= sizes[0]:
